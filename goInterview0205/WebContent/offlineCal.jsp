...
$(function() {
	$(".countview").click(function() {
		...
		var url ="offcallist?year="+year+"&month="+month+"&date="+date+"";
...
<body>
	...
	<div id = "boardSize">
		...
		<%
			...
			String id = dto.getId();
			List<giStdMbrDto> clist = biz.getCalViewListById(id, yyyyMM);
		...
		<div id="calendarsize">
			<table id="calendar">
				<caption>
					<a href="offcalonly?year=<%=year - 1%>&month=<%=month%>">
						&lt;&lt; &nbsp;&nbsp;</a> <a
						href="offcalonly?year=<%=year%>&month=<%=month - 1%>"> &lt;
						&nbsp;</a> <span class="y"
					...
					<a href="offcalonly?year=<%=year%>&month=<%=month + 1%>">&nbsp;
						&gt; </a> <a href="offcalonly?year=<%=year + 1%>&month=<%=month%>">&nbsp;&nbsp;
						&gt;&gt; </a><br /> <br>
				</caption>
...
%>
					...
					style="color:<%=Util.fontColor(i, dayOfWeek)%>; font-size:15pt; padding:5px;"><%=i%></a>
						...
						<div class="offlinelist">
							<%=Util.getCalViewOffline(i, clist)%>
						</div></td>
					<%
